...
class IvDataset:
    def __init__(self, df, types=["fit", "c4c5", "c6", "atm"], option_name = "sp500",iv_spread=False, **kwargs):
        self.df = df
        self.types = types
    ...
    def get_ttm_logm(self, type_, ttm, logm, args):
        ...
        if logm_type in args:
            ...
            if type_ == "fit":
                ...
                logm = self.get_powerspace_logm(logm_min=self.logm_min, logm_max=self.logm_max)
            elif type_ == "c6":
                # logm = np.array([6, 4, 4, 6]) * np.repeat([min(logm), max(logm)], 2)
                logm = np.array([6, 4, 4, 6]) * np.repeat([self.logm_min, self.logm_max], 2)
            elif type_ == "atm":
                logm = np.array([0])
        if expand:
    ...
    def get_ivsmoother_dict(self):
        ...
        # di["flag"] = torch.from_numpy(self.df["abnormal"].values).to(torch.float32)
        di["ttm_c4c5:0"].requires_grad_()
        di["logm_c4c5:0"].requires_grad_()
        di["ttm_c6:0"].requires_grad_()
        di["logm_c6:0"].requires_grad_()
        # di["w_clear"] = torch.from_numpy(self.df["w_clear"].values).to(torch.float32)
        # di["iv_clear"] = torch.from_numpy(self.df["iv_clear"].values).to(torch.float32)
        if self.iv_spread:
