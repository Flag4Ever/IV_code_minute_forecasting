...
class ImpModel(nn.Module):
    def __init__(self, neurons_vec=[40,40,40,40], activation="relu", data_channel=1,
                 noise_std=0.01, penalty=dict(fit=1, c4=10, c5=10, c6=10, atm=0.1), is_layernorm=True,gain_alpha=0.2):
        super().__init__()
        ...
        for n_neurons in neurons_vec:
            ...
            # nn.init.normal_(layer.weight, mean=0, std=math.sqrt(1.0 / (n_input + n_neurons)))
            # nn.init.normal_(layer.bias, mean=0, std=math.sqrt(1.0 / (n_input + n_neurons)))
            fan_in = layer.weight.size(1)
            gain = math.sqrt(2.0 / (1.0+gain_alpha))
        ...
        fan_in = self.output_layer.weight.size(1)
        gain = math.sqrt(2.0 / (1.0+gain_alpha))
        std = gain / math.sqrt(fan_in)
        nn.init.normal_(self.output_layer.weight, mean=0.0, std=std)
        nn.init.constant_(self.output_layer.bias, 0.0)
        # nn.init.normal_(self.output_layer.weight, mean=0, std=math.sqrt(1.0 / (n_input + 1)))
        # nn.init.normal_(self.output_layer.bias, mean=0, std=math.sqrt(1.0 / (n_input + 1)))
