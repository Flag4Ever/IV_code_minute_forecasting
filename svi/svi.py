...
from utils import arbitrage, black_scholes
...
class SVI:
    ...
    @classmethod
    def implied_volatility(cls, z, *params, nu: int = 0, **kwargs) -> float:
        if nu == 0:
            w = cls.implied_variance(z, nu=0, *params, **kwargs)
            w = np.maximum(w, 1e-6)
            return w ** .5
        if nu == 1:
            iv = cls.implied_volatility(z, nu=0, *params, **kwargs)
            dw_dz = cls.implied_variance(z, nu=1, *params, **kwargs)
            return dw_dz / (2 * iv)
        if nu == 2:
            iv = cls.implied_volatility(z, nu=0, *params, **kwargs)
            dw_dz = cls.implied_variance(z, nu=1, *params, **kwargs)
            dw_dzz = cls.implied_variance(z, nu=2, *params, **kwargs)
    ...
    def get_arb_loss(self,r,z,**kwargs):
        iv = self.implied_volatility(z, **kwargs)
        div_dz = self.implied_volatility(z, **kwargs, nu=1)
        div_dzz = self.implied_volatility(z, **kwargs, nu=2)
        d1, d2 = black_scholes.normalizing_transforms(r, z, iv)
        but = arbitrage.butterfly(d1, d2, iv, div_dz, div_dzz)
        return {
            "l_butterfly": np.maximum(-but, 0).sum(),
            "butterfly": but.sum(),
        }
