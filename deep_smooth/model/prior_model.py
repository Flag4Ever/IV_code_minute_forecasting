...
class ATMInterpolator:
    ...
    def _fit(self):
        # 即使是单调的情况,在数值的范围超过取值范围后，也会出现负值的w_atm
        if np.all(np.diff(self.w_atm) >= 0):
            ttm_atm = np.concatenate([[0], self.ttm_atm])
            w_atm = np.concatenate([[0], self.w_atm])
            fit = CubicSpline(ttm_atm, w_atm,bc_type='natural')
            # print(f"fit {fit(self.ttm_grid)}")
            ...
            model = Ridge(alpha=0.5).fit(ttm_atm_poly,np.log(self.w_atm))
    ...
    def forward(self, logm, w_atm):
        ...
        if self.prior == "svi":
            if self.phi_fun == "heston_like":
                ...
                phi = 1 / (lambda_ * w_atm+1e-6) * (1 - (1 - torch.exp(-lambda_ * w_atm)) / (lambda_ * w_atm+1e-6))
            elif self.phi_fun == "power_law":
                eta = torch.exp(self.eta_trans)
                gamma = torch.sigmoid(self.gamma_trans)
                # print(f"w_atm {w_atm}")
                phi = eta / (torch.pow(w_atm, gamma) * torch.pow(1 + w_atm, 1 - gamma)+1e-6)
            else:
                raise ValueError("Incorrect function for phi")
